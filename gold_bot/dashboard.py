"""
dashboard.py - Streamlit dashboard for the Gold Bot.

Shows: balance, win rate, PnL, equity curve, recent trades, decisions.

Run: streamlit run gold_bot/dashboard.py --server.port 8052
"""
...
from gold_bot.config import DB_PATH, INITIAL_BALANCE, DASHBOARD_UPDATE_INTERVAL
...
def run_dashboard():
    ...
    try:
        ...
        print("Then: streamlit run gold_bot/dashboard.py --server.port 8052")
        return

    st.set_page_config(
        page_title="GOLD Golden Cross Bot - Dashboard",
        page_icon="$",
        layout="wide",
    )

    st.title("GOLD Golden Cross Bot - Dashboard")
    st.markdown("---")
    ...
    with col_left:
        ...
        if not equity_df.empty:
            ...
            st.info("No equity data yet. The bot needs to execute trades first.")
    ...
    if not trades_df.empty:
        display_cols = [
            "id", "direction", "entry_price", "exit_price",
    ...
    # ── Bot Decisions ─────────────────────────────────────────
    st.subheader("Recent Signals (Golden Cross)")
    decisions_df = load_decisions(20)
    if not decisions_df.empty:
        display_cols = []
        if "timestamp" in decisions_df.columns:
            decisions_df["time"] = pd.to_datetime(
                decisions_df["timestamp"]
            ).dt.strftime("%H:%M")
        ...
        if "confidence" in decisions_df.columns:
            decisions_df["conf"] = (
                (decisions_df["confidence"] * 100).round(0).astype(str) + "%"
            )
            display_cols.append("conf")
